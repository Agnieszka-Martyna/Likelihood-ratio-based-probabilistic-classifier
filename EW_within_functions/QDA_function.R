QDA_function = function(train_set,y.mean.1,variables_qda)
{
  data=train_set[,c(1,variables_qda)]
  ...
  # 
  library("MASS")
  qda = qda(Factor~.,data=data,prior=rep(1,times=length(unique(data$Factor)))/length(unique(data$Factor)))
  ...
  new.object.transformed = new.object - matrix(mean.data,ncol=length(variables_qda),nrow=nrow(new.object),byrow=T)
  predicted = predict(qda,new.object.transformed) ####new  obejct must be manually transofrmed as training set!!!!!!!
  # predicted$x
  ...
  # predicted.mean3 = predict(lda,as.data.frame(t(apply(data[which(data$Factor==cats[3]),-1],2,mean))))$x
  # 
  predicted = predicted$posterior
  # prior=c(1/nclass,1/nclass,1/nclass)
  ...
  # s3 = var(predicted.training[which(data$Factor==cats[3]),])
  # n3 = length(predicted.training[which(data$Factor==cats[3]),])
  # pooled.cov = ((n1-1)*s1+(n2-1)*s2+(n3-1)*s3)/(n1+n2+n3-3)
  ...
  # post3=prior[3]*dmvnorm(predicted$x,mean=predicted.mean3,sigma=as.matrix(pooled.cov))
  # probs1 = post1/(post1+post2+post3)
  # probs2 = post2/(post1+post2+post3)
  # probs3 = post3/(post1+post2+post3)
  
 
  
  
  return(list(probabilities=predicted))
